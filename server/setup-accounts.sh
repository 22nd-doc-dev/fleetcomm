...
# the staged repository. DNS for PUBLIC_HOST must already point at this host.
set -euo pipefail

PUBLIC_HOST="${1:?usage: setup-accounts.sh <public-host> <letsencrypt-email>}"
...
certbot --nginx --non-interactive --agree-tos --redirect --email "$LE_EMAIL" -d "$PUBLIC_HOST"
...
#!/usr/bin/env bash
set -e
install -m 0640 -o root -g mumble-server /etc/letsencrypt/live/${PUBLIC_HOST}/fullchain.pem /etc/mumble-server/fleetcomm-tls/fullchain.pem
install -m 0640 -o root -g mumble-server /etc/letsencrypt/live/${PUBLIC_HOST}/privkey.pem /etc/mumble-server/fleetcomm-tls/privkey.pem
...
if command -v ufw >/dev/null && ufw status | grep -q "Status: active"; then
  ...
  ufw allow 80/tcp; ufw allow 443/tcp; ufw allow 64738/tcp; ufw allow 64738/udp
fi

rm -rf "$SECRETS_DIR"
curl --fail --silent --show-error "https://${PUBLIC_HOST}/api/health"
echo
echo "FleetComm accounts and relay TLS are live at ${PUBLIC_HOST}."
